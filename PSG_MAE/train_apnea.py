...
from data.dataset_apnea import (
...
from model.Classification_Head import Classification_Head
...
def train_one_fold(fold, train_subjects, test_subjects, args):
    ...
    encoder = PSG_Encoder()
    encoder.load_state_dict(torch.load(args.encoder_weight, map_location='cuda' if torch.cuda.is_available() else 'cpu'))
    if args.freeze:
        for param in encoder.parameters():
            param.requires_grad = False

    classifier = Classification_Head(num_classes=2)
    ...
    acc = accuracy_score(targets, preds)
    f1 = f1_score(targets, preds, average='macro')
    report = classification_report(targets, preds, target_names=[LABEL_DICT[i] for i in range(2)], digits=4)
    cm = confusion_matrix(targets, preds)
    plot_confusion_matrix(cm, [LABEL_DICT[i] for i in range(2)], os.path.join(args.save_dir, f"fold_{fold+1}_confusion_matrix.png"))
...
def main():
    ...
    with open(os.path.join(args.save_dir, "final_result.txt"), 'w') as f:
        ...
        report_dict = classification_report(
            y_true=np.repeat(np.arange(2), total_cm.sum(axis=1)),
            y_pred=np.concatenate([np.repeat(np.arange(2), row) for row in total_cm]),
            target_names=[LABEL_DICT[i] for i in range(2)],
            digits=4,
            output_dict=True
        )
    ...
    plot_confusion_matrix(total_cm, [LABEL_DICT[i] for i in range(2)], os.path.join(args.save_dir, "final_confusion_matrix.png"))
